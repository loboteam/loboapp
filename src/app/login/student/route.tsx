...
export const POST = async (req: Request) => {
    ...
    const res = await sb.schema("public").from("students").select("id:matricula, name:nombre").eq("matricula", id).eq("phash",pwdHash);
    const isadm = await sb.schema("public").from("admins").select("matricula").eq("is", 'student').eq("matricula", id);
