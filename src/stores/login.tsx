...
const LoginContext: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const logIn = (staff: boolean=false) => {
        if (id.length > 1 && pwd.length > 1)
            return sha512(pwd).then(pwdHash => fetch(ENDPOINTS.login[staff ? "staff" : "student"], { method: "POST", body: JSON.stringify({ id, pwdHash }) })).then(r => r.json()).then((r: User | undefined) => setUser(r)).catch(e => console.error("Ack! Bad login action! Got: ", e));
       throw new Error("incomplete login info!");
    }

    return <LoginContext_Bare.Provider value={{id, setId, pwd, setPwd, logIn}}>{children}</LoginContext_Bare.Provider>
};
